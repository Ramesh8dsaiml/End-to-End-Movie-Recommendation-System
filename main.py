...
BASE_URL = "https://tmdb-movies-and-tv-shows-api.p.rapidapi.com"
TMDB_IMG_500 = "https://image.tmdb.org/t/p/w500"

if not RAPIDAPI_KEY:
    # Don't crash import-time in production if you prefer; but for you better fail early:
    raise RuntimeError("RAPIDAPI_KEY missing. Put it in .env as RAPIDAPI_KEY=your_key")
HEADERS = {
    "X-RapidAPI-Key": RAPIDAPI_KEY,
    "X-RapidAPI-Host": "tmdb-movies-and-tv-shows-api.p.rapidapi.com"
}
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    q = dict(params)
    q["api_key"] = RAPIDAPI_KEY

    try:
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.get(f"{BASE_URL}{path}", params=q)
...
async def root():
    """
    Root endpoint - API welcome message and documentation.
    
    Returns basic information about the Movie Recommender API.
    ...
    return {
        ...
        "endpoints": {
            ...
            "details": "/movie/id/{tmdb_id}",
            "recommendations": {
                "genre": "/recommend/genre",
                "tfidf": "/recommend/tfidf",
            }
        }
    }
...
async def health():
    ...
    Returns: status and whether pickles are loaded
    """
    return {
        "status": "ok",
        "service": "Movie Recommender API",
        "ready": all([df is not None, tfidf_matrix is not None])
    }
